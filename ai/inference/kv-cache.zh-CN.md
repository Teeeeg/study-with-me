...
1. 先理解 KV Cache 的作用，以及 Prefill 和 Decode 的基本流程；
...
### 5.1 MHA（多头注意力）：每个 Q head 配一组 K/V
...
### 5.2 GQA（分组查询注意力）：多个 Q head 共享一组 K/V
...
### 5.3 MQA（多查询注意力）：所有 Q head 共享唯一一组 K/V
...
#### 5.5.2 HCA：混合使用多种压缩策略

不同层可以采用不同压缩强度，并额外保留最近一段 token 的局部窗口：摘要覆盖长历史，窗口保留近期细节。

![HCA 混合局部窗口与不同强度的全局压缩](../../.asset/kv-cache/hca-layer-strategy.svg)

这类缓存需要按层分别估算：把各层的摘要、局部窗口及额外索引相加，而不是直接套用第 3 节的统一 KV-head 公式。减少历史条目会改变模型能读取的信息，需要模型设计和训练配合，不是可以随意丢弃旧 KV 的通用开关。
...
## 6. PagedAttention：分页管理 KV Cache
### 6.1 连续分配为什么浪费显存
...
### 6.5 分页与 continuous batching 的协同
...
### 6.6 Automatic Prefix Caching 如何复用 block
...
## 7. Chunked Prefill：分块调度长 Prompt
...
#### 7.1.2 传统 Prefill 对 KV Cache 的双重冲击
...
### 7.2 Chunked Prefill 对 KV Cache 的三个改变

#### 7.2.1 Block 分配：从一次性到增量式
...
#### 7.2.2 Prefix Caching：只在第一个 chunk 查找
...
#### 7.3.2 long_prefill_token_threshold：长 prompt 的特殊对待
...
### 8.3 与 vLLM Automatic Prefix Caching 的区别
...
## 9. 分布式 KV Cache 与 Prefill-Decode 分离

### 9.1 多张卡上，KV Cache 如何分布
...
### 9.2 Offloading：把暂时不用的 KV 放到更便宜的地方
