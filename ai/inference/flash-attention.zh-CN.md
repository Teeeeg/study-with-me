...
![GPU 存储层级金字塔](../../.asset/flash-attention/flashattn_banner.jpg)
...
图中红色箭头表示完整 $N\times N$ 中间矩阵跨越 HBM 与片上空间：标准方式需要写 S、读 S、写 P、读 P；FlashAttention 只跨层读入 Q/K/V tile、写回最终 O，S/P tile 始终在片上产生并立即消费。这里比较的是**物化中间矩阵的朴素实现**，不是说所有框架默认都走上面的路径。
...
Tiling 把 Q/K/V 切成能放入 Shared Memory 和寄存器的小块。数据从 HBM 搬上来后，局部分数计算、Softmax 和 V 的加权累积连续完成，让同一小块在片上被反复使用，避免每个算子都重新读写 HBM。
...
因此，Online Softmax 只需跨块保留每行的 $m$、$\ell$ 和 $u$。当前 S/P tile 用完即可丢弃，不必写回 HBM；处理完所有块后再计算 $o=u/\ell$。
